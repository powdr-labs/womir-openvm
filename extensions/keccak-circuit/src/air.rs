...
impl<AB: InteractionBuilder> Air<AB> for KeccakVmAir {
    fn eval(&self, builder: &mut AB) {
        ...
        let start_read_timestamp =
            self.eval_instruction(builder, local, &mem.fp_aux, &mem.register_aux);
...
impl KeccakVmAir {
    ...
    pub fn constrain_block_transition<AB: AirBuilder>(
        ...
        block_transition.assert_eq(local.instruction.fp, next.instruction.fp);
        ...
        // `fp, dst, src, len` and block input bytes.
    ...
    /// First reads FP from FP_AS, then does memory read in addr space 1 to get
    /// `dst, src, len` from memory at register addresses offset by FP.
    ///
    ...
    pub fn eval_instruction<AB: InteractionBuilder>(
        ...
        register_aux: &[MemoryReadAuxCols<AB::Var>; KECCAK_REGISTER_READS - 1],
    ) -> AB::Expr {
    ...
    pub fn timestamp_change<T: FieldAlgebra>(len: impl Into<T>) -> T {
        // actual number is ceil(len / 136) * (4 + 17) + KECCAK_DIGEST_WRITES
