...
/// Columns for KECCAK256 instruction parsing.
/// Includes columns for instruction execution and register reads.
/// Register addresses are offset by the frame pointer (fp).
...
pub struct KeccakMemoryCols<T> {
    ...
    pub register_aux: [MemoryReadAuxCols<T>; KECCAK_REGISTER_READS - 1],
...
impl<T: Copy> KeccakInstructionCols<T> {
        ...
        T: Into<AB::Expr>,
    {
        ...
        builder.assert_eq(self.fp, other.fp);
