...
use crate::{
    adapters::decompose, FrameBridge, FrameBus, FrameState, VmAdapterChipWom, WomBridge, WomController, WomRecord
};
    ...
    fn eval(
        ...
        self.wom_bridge
            .write(
                ...
                [local.next_frame_ptr.into(), AB::Expr::ZERO, AB::Expr::ZERO, AB::Expr::ZERO],
                local.write_mult,
            )
...
impl<F: PrimeField32> VmAdapterChipWom<F> for AllocateFrameAdapterChipWom {
    ...
    fn preprocess(
        ...
        let amount = if use_reg == F::ZERO {
            ...
            let (_, reg_data) = wom.read::<1>(amount_reg + fp_f);
    ...
    fn postprocess(
        ...
        if enabled != F::ZERO {
            write_result = Some(wom.write(
                ...
                [F::from_canonical_u32(read_record.allocated_ptr), F::ZERO, F::ZERO, F::ZERO],
            ));
        }
