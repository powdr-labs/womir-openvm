...
/// is an immediate).
pub struct WomBaseAlu64AdapterChip<F: Field> {
    pub air: WomBaseAlu64AdapterAir,
    bitwise_lookup_chip: SharedBitwiseOperationLookupChip<RV32_CELL_BITS>,
    _marker: PhantomData<F>,
}

impl<F: PrimeField32> WomBaseAlu64AdapterChip<F> {
    pub fn new(
...
#[serde(bound = "F: Field")]
pub struct WomBaseAlu64ReadRecord<F: Field> {
...
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(bound = "F: Field")]
pub struct WomBaseAlu64WriteRecord<F: Field> {
...
pub struct WomBaseAlu64AdapterCols<T> {
    ...
    pub rs2_as: T,
    pub reads_aux: [MemoryReadAuxCols<T>; 4],
    pub writes_aux: MemoryWriteAuxCols<T, { RV32_REGISTER_NUM_LIMBS * 2 }>,
}

#[allow(dead_code)]
#[derive(Clone, Copy, Debug, derive_new::new)]
pub struct WomBaseAlu64AdapterAir {
...
impl<F: Field> BaseAir<F> for WomBaseAlu64AdapterAir {
    fn width(&self) -> usize {
        WomBaseAlu64AdapterCols::<F>::width()
    }
}

impl<F: Field> ColumnsAir<F> for WomBaseAlu64AdapterAir {
    fn columns(&self) -> Option<Vec<String>> {
        WomBaseAlu64AdapterCols::<F>::struct_reflection()
    }
}

impl<AB: InteractionBuilder> VmAdapterAir<AB> for WomBaseAlu64AdapterAir {
    ...
    fn eval(
        ...
        let local: &WomBaseAlu64AdapterCols<_> = local.borrow();
    ...
    fn get_from_pc(&self, local: &[AB::Var]) -> AB::Var {
        let cols: &WomBaseAlu64AdapterCols<_> = local.borrow();
        cols.from_state.pc
    }
}

impl<F: PrimeField32> VmAdapterChipWom<F> for WomBaseAlu64AdapterChip<F> {
    type ReadRecord = WomBaseAlu64ReadRecord<F>;
    type WriteRecord = WomBaseAlu64WriteRecord<F>;
    type Air = WomBaseAlu64AdapterAir;
    type Interface = BasicAdapterInterface<
        F,
    ...
    fn preprocess(
        ...
        let rs1 = memory.read::<{ RV32_REGISTER_NUM_LIMBS * 2 }>(d, b + fp_f);
        let (rs2, rs2_data, rs2_imm) = if e.is_zero() {
            ...
            let rs2_read = memory.read::<{ RV32_REGISTER_NUM_LIMBS * 2 }>(e, c + fp_f);
    ...
    fn generate_trace_row(
        ...
        let row_slice: &mut WomBaseAlu64AdapterCols<_> = row_slice.borrow_mut();
