...
impl<AB: InteractionBuilder> VmAdapterAir<AB> for JaafAdapterAirWom {
    ...
    fn eval(
        ...
        // read fp
        self.wom_bridge
            .read(local.fp_read_reg + local.from_frame.fp, [local.fp_read_val.into(), AB::Expr::ZERO, AB::Expr::ZERO, AB::Expr::ZERO])
        ...
        self.wom_bridge
            .read(
                ...
                [local.pc_read_val.into(), AB::Expr::ZERO, AB::Expr::ZERO, AB::Expr::ZERO],
            )
        ...
        self.wom_bridge
            .write(
                ...
                [local.pc_save_val.into(), AB::Expr::ZERO, AB::Expr::ZERO, AB::Expr::ZERO],
                local.pc_write_mult,
            )
        ...
        self.wom_bridge
            .write(
                ...
                [local.fp_save_val.into(), AB::Expr::ZERO, AB::Expr::ZERO, AB::Expr::ZERO],
                local.fp_write_mult,
            )
    ...
    fn preprocess(
        ...
        let (pc_source_record, pc_source_data) = match local_opcode {
            JaafOpcode::RET | JaafOpcode::CALL_INDIRECT => {
                ...
                let pc_source = wom.read::<1>(c + fp_f);
        ...
        let fp_source = wom.read::<1>(e + fp_f);

        Ok((
            [[pc_source_data, F::ZERO, F::ZERO, F::ZERO], [fp_source.1[0], F::ZERO, F::ZERO, F::ZERO]],
            JaafReadRecord {
        ...
        let (rd1, rd2) = if enabled != F::ZERO {
            match local_opcode {
                ...
                JaafOpcode::JAAF_SAVE => {
                    ...
                    let rd2 = wom.write(b + F::from_canonical_u32(to_fp), [F::from_canonical_u32(from_fp), F::ZERO, F::ZERO, F::ZERO]);
                    (None, Some(rd2))
                }
                JaafOpcode::CALL | JaafOpcode::CALL_INDIRECT => {
                    // Save both pc to rd1 (a field) and fp to rd2 (b field)
                    let rd1 = wom.write(a + F::from_canonical_u32(to_fp), [F::from_canonical_u32(from_pc), F::ZERO, F::ZERO, F::ZERO]);
                    let rd2 = wom.write(b + F::from_canonical_u32(to_fp), [F::from_canonical_u32(from_fp), F::ZERO, F::ZERO, F::ZERO]);
                    (Some(rd1), Some(rd2))
                }
            }
