...
// Uses CrushCallAdapter (with frame pointer) and inlined CallCore logic.
#include "launcher.cuh"
#include "primitives/buffer_view.cuh"
#include "primitives/constants.h"
#include "primitives/trace_access.h"
#include "crush/adapters/call.cuh"
...
template <typename T> struct CrushCallCols {
    CrushCallAdapterCols<T> adapter;
    CallCoreCols<T> core;
};

struct CrushCallRecord {
    CrushCallAdapterRecord adapter;
    CallCoreRecord core;
};

__global__ void crush_call_tracegen(
    Fp *trace,
    size_t height,
    size_t width,
    DeviceBufferConstView<CrushCallRecord> records,
    ...
    if (idx < records.len()) {
        ...
        auto core = CallCore();
        core.fill_trace_row(row.slice_from(COL_INDEX(CrushCallCols, core)), record.core);
    } else {
        row.fill_zero(0, sizeof(CrushCallCols<uint8_t>));
    }
}

extern "C" int _crush_call_tracegen(
    Fp *d_trace,
    size_t height,
    size_t width,
    DeviceBufferConstView<CrushCallRecord> d_records,
    size_t pointer_max_bits,
    ...
    assert(width == sizeof(CrushCallCols<uint8_t>));
    auto [grid, block] = kernel_launch_params(height);

    crush_call_tracegen<<<grid, block>>>(
        d_trace,
        height,
        width,
        d_records,
