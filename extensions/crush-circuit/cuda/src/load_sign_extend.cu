...
// Uses CrushLoadStoreAdapter (with frame pointer) instead of Rv32LoadStoreAdapter
#include "launcher.cuh"
#include "primitives/buffer_view.cuh"
#include "primitives/constants.h"
#include "primitives/histogram.cuh"
#include "primitives/trace_access.h"
#include "crush/adapters/loadstore.cuh"
...
template <typename T> struct CrushLoadSignExtendCols {
    CrushLoadStoreAdapterCols<T> adapter;
    LoadSignExtendCoreCols<T, RV32_REGISTER_NUM_LIMBS> core;
};

struct CrushLoadSignExtendRecord {
    CrushLoadStoreAdapterRecord adapter;
    LoadSignExtendCoreRecord<RV32_REGISTER_NUM_LIMBS> core;
};

__global__ void crush_load_sign_extend_tracegen(
    Fp *trace,
    size_t height,
    size_t width,
    DeviceBufferConstView<CrushLoadSignExtendRecord> records,
    ...
    if (idx < records.len()) {
        ...
        auto adapter = CrushLoadStoreAdapter(
            pointer_max_bits,
        ...
        core.fill_trace_row(row.slice_from(COL_INDEX(CrushLoadSignExtendCols, core)), record.core);
    } else {
        row.fill_zero(0, sizeof(CrushLoadSignExtendCols<uint8_t>));
    }
}

extern "C" int _crush_load_sign_extend_tracegen(
    Fp *__restrict__ d_trace,
    size_t height,
    size_t width,
    DeviceBufferConstView<CrushLoadSignExtendRecord> d_records,
    size_t pointer_max_bits,
    ...
    assert(width == sizeof(CrushLoadSignExtendCols<uint8_t>));
    auto [grid, block] = kernel_launch_params(height, 512);

    crush_load_sign_extend_tracegen<<<grid, block>>>(
        d_trace,
        height,
        width,
        d_records,
