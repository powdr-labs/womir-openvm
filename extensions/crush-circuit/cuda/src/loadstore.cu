...
__global__ void crush_load_store_tracegen(
    ...
    if (idx < records.len()) {
        ...
        core.fill_trace_row(row.slice_from(COL_INDEX(CrushLoadStoreCols, core)), record.core);
    } else {
        row.fill_zero(0, sizeof(CrushLoadStoreCols<uint8_t>));
    }
}

extern "C" int _crush_load_store_tracegen(
    Fp *d_trace,
    size_t height,
    size_t width,
    DeviceBufferConstView<CrushLoadStoreRecord> d_records,
    size_t pointer_max_bits,
    ...
    assert(width == sizeof(CrushLoadStoreCols<uint8_t>));
    auto [grid, block] = kernel_launch_params(height);

    crush_load_store_tracegen<<<grid, block>>>(
        d_trace,
        height,
        width,
        d_records,
