...
struct CrushBaseAluAdapterCols {
...
struct CrushBaseAluAdapterRecord {
    uint32_t from_pc;
    uint32_t fp;
...
template <size_t NUM_READ_OPS, size_t NUM_WRITE_OPS>
struct CrushBaseAluAdapter {
    MemoryAuxColsFactory mem_helper;
    BitwiseOperationLookup bitwise_lookup;

    template <typename T>
    using Cols = CrushBaseAluAdapterCols<T, NUM_READ_OPS, NUM_WRITE_OPS>;
    using Record = CrushBaseAluAdapterRecord<NUM_READ_OPS, NUM_WRITE_OPS>;

    __device__ CrushBaseAluAdapter(
