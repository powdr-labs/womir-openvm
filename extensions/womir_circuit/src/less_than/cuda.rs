...
use openvm_rv32im_circuit::{LessThanCoreCols, LessThanCoreRecord};
...
#[derive(new)]
pub struct LessThan64ChipGpu {
    pub range_checker: Arc<VariableRangeCheckerChipGPU>,
    pub bitwise_lookup: Arc<BitwiseOperationLookupChipGPU<RV32_CELL_BITS>>,
    pub timestamp_max_bits: usize,
}

impl Chip<DenseRecordArena, GpuBackend> for LessThan64ChipGpu {
    fn generate_proving_ctx(&self, arena: DenseRecordArena) -> AirProvingContext<GpuBackend> {
        const RECORD_SIZE: usize = size_of::<(
            BaseAluAdapterRecordDifferentInputsOutputs<W64_REG_OPS, W32_REG_OPS>,
            LessThanCoreRecord<W64_NUM_LIMBS, RV32_CELL_BITS>,
        )>();
        let records = arena.allocated();
        if records.is_empty() {
            return get_empty_air_proving_ctx::<GpuBackend>();
        ...
        let trace_width = LessThanCoreCols::<F, W64_NUM_LIMBS, RV32_CELL_BITS>::width()
            + BaseAluAdapterColsDifferentInputsOutputs::<F, W64_REG_OPS, W32_REG_OPS>::width();
        let trace_height = next_power_of_two_or_zero(records.len() / RECORD_SIZE);
