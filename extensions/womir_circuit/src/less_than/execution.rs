...
macro_rules! dispatch {
    ($execute_impl:ident, $is_imm:ident, $opcode:expr, $offset:expr) => {
        Ok(
            match (
                $is_imm,
                LessThanOpcode::from_usize($opcode.local_opcode_idx($offset)),
            ) {
                (true, LessThanOpcode::SLT) => $execute_impl::<_, _, true, true>,
                (false, LessThanOpcode::SLT) => $execute_impl::<_, _, false, true>,
                (true, LessThanOpcode::SLTU) => $execute_impl::<_, _, true, false>,
                (false, LessThanOpcode::SLTU) => $execute_impl::<_, _, false, false>,
            },
        )
    };
}
...
>(
    ...
    let rd = if cmp_result { 1u32 } else { 0u32 };
...
>(
    ...
    unsafe {
        ...
        execute_e12_impl::<F, CTX, IS_IMM, IS_SIGNED>(pre_compute, exec_state);
    }
}
...
>(
    ...
    unsafe {
        ...
        execute_e12_impl::<F, CTX, IS_IMM, IS_SIGNED>(&pre_compute.data, exec_state);
    }
}
