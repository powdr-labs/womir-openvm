...
use super::core::JaafCoreRecord;

/// JAAF executor. Wraps the adapter executor.
#[derive(Clone, Copy)]
pub struct JaafExecutor<A> {
    pub adapter: A,
    pub offset: usize,
}

impl<A> JaafExecutor<A> {
    pub fn new(adapter: A, offset: usize) -> Self {
        Self { adapter, offset }
    }
}

impl<A> std::ops::Deref for JaafExecutor<A> {
    type Target = A;
...
/// Pre-computed data for JAAF instruction execution (E1/E2 stages).
#[derive(AlignedBytesBorrow, Clone)]
#[repr(C)]
pub(super) struct JaafPreCompute {
...
impl<F, A, RA> PreflightExecutor<F, RA> for JaafExecutor<A>
where
    F: PrimeField32,
    A: 'static
    ...
    for<'buf> RA: RecordArena<
            ...
            (A::RecordMut<'buf>, &'buf mut JaafCoreRecord),
        >,
{
    fn get_opcode_name(&self, opcode: usize) -> String {
        let local_idx = VmOpcode::from_usize(opcode).local_opcode_idx(JaafOpcode::CLASS_OFFSET);
        format!("{:?}", JaafOpcode::from_usize(local_idx))
    }
...
impl<F: PrimeField32, A> InterpreterExecutor<F> for JaafExecutor<A> {
    #[inline(always)]
    fn pre_compute_size(&self) -> usize {
        size_of::<JaafPreCompute>()
    }
    ...
    {
        let data: &mut JaafPreCompute = data.borrow_mut();
        let local_idx = inst.opcode.local_opcode_idx(JaafOpcode::CLASS_OFFSET);
        let opcode = JaafOpcode::from_usize(local_idx);
...
impl<F: PrimeField32, A> InterpreterMeteredExecutor<F> for JaafExecutor<A> {
    #[inline(always)]
    fn metered_pre_compute_size(&self) -> usize {
        size_of::<E2PreCompute<JaafPreCompute>>()
    }
    ...
    {
        let data: &mut E2PreCompute<JaafPreCompute> = data.borrow_mut();
...
#[inline(always)]
unsafe fn execute_jaaf_impl<F: PrimeField32, CTX: ExecutionCtxTrait>(
    pre: &JaafPreCompute,
...
unsafe fn execute_e1_impl<F: PrimeField32, CTX: ExecutionCtxTrait>(
    ...
    unsafe {
        let pre: &JaafPreCompute =
            std::slice::from_raw_parts(pre_compute, size_of::<JaafPreCompute>()).borrow();
        execute_jaaf_impl::<F, CTX>(pre, exec_state);
    }
}
...
unsafe fn execute_e2_impl<F: PrimeField32, CTX: MeteredExecutionCtxTrait>(
    ...
    unsafe {
        let pre: &E2PreCompute<JaafPreCompute> =
            std::slice::from_raw_parts(pre_compute, size_of::<E2PreCompute<JaafPreCompute>>())
                .borrow();
        exec_state.ctx.on_height_change(pre.chip_idx as usize, 1);
        execute_jaaf_impl::<F, CTX>(&pre.data, exec_state);
    }
}
