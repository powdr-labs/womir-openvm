...
use openvm_rv32im_wom_transpiler::Rv32JumpOpcode;
...
#[derive(Debug)]
pub struct Rv32JumpAdapterChipWom<F: Field> {
    pub air: Rv32JumpAdapterAirWom,
    _marker: PhantomData<F>,
}

impl<F: PrimeField32> Rv32JumpAdapterChipWom<F> {
    pub fn new(
...
pub struct Rv32JumpAdapterColsWom<T> {
...
pub struct Rv32JumpAdapterAirWom {
    pub(super) memory_bridge: MemoryBridge,
    pub(super) execution_bridge: ExecutionBridge,
}

impl<F: Field> BaseAir<F> for Rv32JumpAdapterAirWom {
    fn width(&self) -> usize {
        Rv32JumpAdapterColsWom::<F>::width()
    }
}

impl<F: Field> ColumnsAir<F> for Rv32JumpAdapterAirWom {
    fn columns(&self) -> Option<Vec<String>> {
        Rv32JumpAdapterColsWom::<F>::struct_reflection()
    }
}

impl<AB: InteractionBuilder> VmAdapterAir<AB> for Rv32JumpAdapterAirWom {
    ...
    fn eval(
        ...
        let local_cols: &Rv32JumpAdapterColsWom<AB::Var> = local.borrow();
    ...
    fn get_from_pc(&self, local: &[AB::Var]) -> AB::Var {
        let cols: &Rv32JumpAdapterColsWom<_> = local.borrow();
        cols.from_state.pc
    }
}

impl<F: PrimeField32> VmAdapterChipWom<F> for Rv32JumpAdapterChipWom<F> {
    type ReadRecord = Rv32JumpReadRecord;
    type WriteRecord = Rv32JumpWriteRecord;
    type Air = Rv32JumpAdapterAirWom;
    type Interface = BasicAdapterInterface<
        F,
    ...
    fn preprocess(
        ...
        let (condition_record, condition_data) = match local_opcode {
            Rv32JumpOpcode::JUMP_IF | Rv32JumpOpcode::JUMP_IF_ZERO => {
    ...
    fn generate_trace_row(
        ...
        let adapter_cols: &mut Rv32JumpAdapterColsWom<_> = row_slice.borrow_mut();
