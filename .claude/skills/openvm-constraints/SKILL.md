...
This project adds frame pointer (`fp`) support. Read `extensions/crush-circuit/src/execution.rs` for:
...
See `extensions/crush-circuit/src/adapters/alu.rs`. Read it completely when writing adapters, to have a reference.
...
| Component | Path |
|-----------|------|
| ExecutionBridge/State/FpBus | `extensions/crush-circuit/src/execution.rs` |
| ALU Adapter | `extensions/crush-circuit/src/adapters/alu.rs` |
...
All chips are instantiated in `extensions/crush-circuit/src/extension/mod.rs`, in `CrushCpuProverExt::extend_prover`.